...
const Sidebar = ({open = false,setOpen}) => {
    ...
    var SidebarMenu = ({nama="Belum diisi",icon="download"}) => {
      return <View style={[tw`flex flex-row items-center text-xl p-3`]}>
  ...
  return (
    <Animated.View style={[tw`absolute left-0 h-full bg-white w-6/8`,{marginLeft:fadeAnim}]}>
      ...
      <View style={[tw`w-full p-2 bg-sky-600  flex-row justify-between items-center`]}>
        <View style={tw`flex-row items-center`}>  
          <Image source={Logo} style = {[tw``,{ width: 35, height: 25 }]}/>
      ...
      <SidebarMenu nama='Back to home' icon='home'/>
    </Animated.View>
