...
const BottomMenu = ({menuOpen,setAttributeOpen,setMenuOpen,editBidang,deleteBidang,setSelectBidang}) => {
  ...
  return (
    <Modal
      ...
      <View style={tw`absolute bottom-0 bg-white w-full`}>
        <View style={tw`w-full flex-row py-2 justify-around items-center bg-white`}>
            <ItemMenu 
                icon={<IconMaterial name='vector-polyline-edit' color={"black"} size={27}/>} 
                nama="Edit"
                press={()=>{
                    editBidang()
                    setMenuOpen(false)
            ...
            />
            <ItemMenu 
                icon={<IconMaterial name='window-close' color={"black"} size={27}/>} 
                nama="Close"
                press={()=>{
                    setMenuOpen(false)
